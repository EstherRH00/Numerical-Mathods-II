...
double itera_gs(double h, double ** mat, double coeficients[], int n);
...
int main () {
	...
	while(k < max_iteracions && error > tolerancia){
		error = itera_gs(h, arr, coeficients, n);
		k++;
		if(k%1000 == 0)
...
/* Desa resultats en un arxiu: gauss_seidel_result.txt*/
void print_file(double** arr, int n, double h){
	int i, j;
	FILE * f = fopen("gauss_seidel_result.txt", "w");
	fprintf(f, "x\ty\tu\n");
	for (i = 0; i < (n+2); i++){
        for (j = 0; j < (n+2); j++){
