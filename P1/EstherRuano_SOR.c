...
double itera_SOR(double h, double ** mat, double coeficients[], int n, double w);
...
int main () {
	...
	int n;
	double tolerancia, max_iteracions, w;
	char zeros;
	printf("Introdueix n: \n");
	scanf("%d", &n);
	...
	while(k < max_iteracions && error > tolerancia){
		error = itera_SOR(h, arr, coeficients, n, w);
		k++;
		if(k%1000 == 0)
...
double itera_SOR(double h, double ** mat, double coeficients[], int n, double w) {
	...
	/* Fare mat[i,j] = mat[i,j] + w(D^-1(b -L*mat  -(D+U)*mat) 
	respecte GS, he de modificar perque els termes de la diagonal apareguin*/
	for (i = 1; i < (n+1); i++){
        for (j = 1; j < (n+1); j++){
			double suma = g(i*h, j*h);
			double valor_anterior = mat[i][j];
			
			suma -= coeficients[0]*(mat[i-1][j-1]-mat[i+1][j-1]-mat[i-1][j+1]+mat[i+1][j+1]);
			suma -= coeficients[1]*mat[i][j-1];
			suma -= coeficients[2]*mat[i-1][j];
			suma -= coeficients[3]*mat[i][j+1];
			suma -= coeficients[4]*mat[i+1][j];
			suma -= coeficients[5]*mat[i][j];
			
			mat[i][j] = valor_anterior + (double) w * suma/coeficients[5];
			
			err_tmp = fabs(mat[i][j]-valor_anterior);
...
double g(double x, double y){
	/*return x+y;*/
	/*return 20*pow(x,3)*pow(y,4) + 12*pow(x,5)*pow(y,2);
	*/
...
/* Desa resultats en un arxiu: SOR_result.txt*/
void print_file(double** arr, int n, double h){
	int i, j;
	FILE * f = fopen("SOR_result.txt", "w");
	fprintf(f, "x\ty\tu\n");
	for (i = 0; i < (n+2); i++){
        for (j = 0; j < (n+2); j++){
